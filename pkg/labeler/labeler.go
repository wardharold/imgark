...
func New(logger *sdlog.StackdriverLogger, mc *minio.Client, projectID, serviceAccount, imageTopic, labeledTopic string) (*Labeler, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("image annotator client creation error: %+v", err)
	}
	...
	if !ok {
		...
		if err != nil {
			return nil, fmt.Errorf("subscription creation error: %+v", err)
		}
	}

	labeler.topic = pc.Topic(labeledTopic)
	if labeler.topic == nil {
		return nil, fmt.Errorf("no such topic: %s", labeledTopic)
	}

	ok, err = labeler.topic.Exists(ctx)
	if err != nil {
		logger.LogError(fmt.Sprintf("Unable to determine if pubsub topic %s exists", labeledTopic), err)
		return nil, err
	}
...
func (l Labeler) ReceiveAndProcess(ctx context.Context) {
	...
	case nil, context.Canceled:
		if err = l.sub.Delete(ctx); err != nil {
			l.logger.LogError(fmt.Sprintf("Unable to delete subscription: %s", l.sub.ID()), err)
		}
	default:
		l.logger.LogError("Unable to Receive", err)
	}
}
...
func (l Labeler) labelImage(bucket, image string) ([]string, error) {
	...
	obj, err := l.mc.GetObject(bucket, image, minio.GetObjectOptions{})
	if err != nil {
		return labels, err
	}
